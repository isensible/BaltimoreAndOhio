...
namespace BOA.Domain
{
    ...
    public class Hex
    {
        ...
        {
            return (int) (q.GetHashCode() ^ (r.GetHashCode() + 0x9e3779b9 + (q << 6) + (q >> 2)));
        }
    ...
    public class Layout
    {
        ...
        static public HexPoint HexToPixel(Layout layout, Hex h)
        {
            Orientation M = layout.orientation;
            HexPoint size = layout.size;
            HexPoint origin = layout.origin;
        ...
        static public FractionalHex PixelToHex(Layout layout, HexPoint p)
        {
            Orientation M = layout.orientation;
            HexPoint size = layout.size;
            HexPoint origin = layout.origin;
        ...
        static public HexPoint HexCornerOffset(Layout layout, int corner)
        {
            Orientation M = layout.orientation;
            HexPoint size = layout.size;
        ...
        static public List<HexPoint> PolygonCorners(Layout layout, Hex h)
        {
            List<HexPoint> corners = new List<HexPoint>{};
            HexPoint center = Layout.HexToPixel(layout, h);
            for (int i = 0; i < 6; i++)
            {
                HexPoint offset = Layout.HexCornerOffset(layout, i);
...
// Tests
    ...
    class Tests
    {
        ...
        {
            Hex h = new Hex(3, 4, -7);
            Layout flat = new Layout(Layout.flat, new HexPoint(10, 15), new HexPoint(35, 71));
            Tests.EqualHex("layout", h, FractionalHex.HexRound(Layout.PixelToHex(flat, Layout.HexToPixel(flat, h))));
            Layout pointy = new Layout(Layout.pointy, new HexPoint(10, 15), new HexPoint(35, 71));
